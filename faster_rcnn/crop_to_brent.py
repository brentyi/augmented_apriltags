  ...
  with tf.Session(graph=detection_graph) as sess:
    for f in frames:
      ...
      for n in range(num):
        ...
        top = (minx, miny)
        bot = (maxx, maxy)
        # cv2.rectangle(image_np, top, bot, (0,255,0), 2)
        d = 45
        ...
        dest = cv2.cvtColor(crop_img, cv2.COLOR_RGB2BGR)
        cv2.imwrite('crop/%05d.png'%idx,dest)
        idx +=1
        cv2.rectangle(image_np, (minx_d, miny_d), (maxx_d, maxy_d), (255,255,0), 1)
        try:
